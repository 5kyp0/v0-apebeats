/**
 * Blockchain Data Collector Tests
 * 
 * Tests for the ApeChain data collection functionality
 */
...
describe('BlockchainDataCollector', () => {
  ...
  describe('collectRecentData', () => {
    ...
    it('should handle multiple blocks', async () => {
      const mockResponses = [
        ...
        },
        {
          ...
          result: {
            ...
            timestamp: '0x6578e8c1',
            gasUsed: '0x249f0',
            gasLimit: '0x1c9c380',
            transactions: [
              {
                hash: '0xbcdef01234567890',
      ...
      const result = await collector.collectRecentData(2);

      expect(result).toBeDefined();
      expect(result.length).toBe(2);
      expect(result[0].blockNumber).toBe(12345);
      expect(result[1].blockNumber).toBe(12346);
    });
    ...
    it('should handle API errors', async () => {
      ...
      await expect(collector.collectRecentData(1)).rejects.toThrow('Failed to collect blockchain data');
    });
  ...
  describe('collectAddressData', () => {
    ...
    it('should filter transactions by address', async () => {
      ...
      expect(result).toBeDefined();
      expect(result.length).toBe(1); // Only one transaction should match
      expect(result[0].fromAddress).toBe(addresses[0]);
    });
  });
  ...
  describe('getNetworkStats', () => {
    ...
    it('should handle network stats errors', async () => {
      ...
      await expect(collector.getNetworkStats()).rejects.toThrow('Failed to get network stats');
    });
  });
  ...
  describe('Error Handling', () => {
    ...
    it('should handle malformed API responses', async () => {
      ...
      await expect(collector.collectRecentData(1)).rejects.toThrow();
    });
  });
});
